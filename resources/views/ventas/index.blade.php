...
<div class="nc-card mb-4">
    <form method="GET" action="{{ route('ventas.index') }}" class="row g-2 align-items-end">
        <div class="col-md-3">
            ...
            <input type="date" name="fecha_desde" value="{{ request('fecha_desde', now()->startOfMonth()->format('Y-m-d')) }}" class="form-control form-control-sm">
        </div>
        <div class="col-md-3">
            <label class="form-label">Hasta</label>
            <input type="date" name="fecha_hasta" value="{{ request('fecha_hasta', now()->format('Y-m-d')) }}" class="form-control form-control-sm">
        </div>
        <div class="col-md-3">
...
<div class="nc-card">
    <div class="table-responsive">
        <table class="table table-hover nc-table mb-0 align-middle">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span class="badge-natural">{{ $venta->metodo_pago }}</span>
                    </td>
                    <td class="text-center">
                        @if($venta->estado === 'completada')
                            <span class="badge-stock-ok">Completada</span>
                        @elseif($venta->estado === 'anulada')
                            <span class="badge-stock-zero">Anulada</span>
                        @else
                            <span class="badge-stock-low">Pendiente</span>
                        @endif
                    </td>
                    <td class="text-end px-4 fw-bold" style="color:var(--neon);">
                        S/ {{ number_format($venta->total,2) }}
                    </td>
                    <td class="px-2">
                        <a href="{{ route('boletas.show', $venta) }}" class="btn btn-secondary btn-sm" title="Ver boleta">
            ...
            <tfoot>
                <tr style="background: rgba(40,199,111,0.10); border-top: 1px solid rgba(40,199,111,0.20);">
                    <td colspan="6" class="px-4 py-3 text-end fw-bold" style="color: var(--text-sec); font-size:13px; text-transform:uppercase; letter-spacing:0.5px;">
                        Total Período:
                    </td>
                    <td class="text-end px-4 fw-bold" style="color: var(--neon); font-size:17px;">
